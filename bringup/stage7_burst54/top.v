// =============================================================================
// ETAPA 7 — CAPTURA EN RÁFAGA @ 54 MSPS (PLL)
// Igual que la etapa 6 pero el reloj sube a 54 MHz vía PLL → muestreo a
// 54 MSPS (18.5 ns/muestra). Duplica los puntos sobre el evento: un pulso de
// ~104 ns pasa de ~3 a ~6 muestras.
//
//   N = 270 muestras @ 54 MSPS  →  ventana = 5 µs
//   Frame: [0xAA 0x55 0xAA 0x55] + 270×uint16 LE (10-bit) = 544 B
//   UART 115200 (CLK_PER_BIT = 54e6/115200 ≈ 468, error 0.16% → ok)
//
// NOTA: 54 MSPS sigue SIN resolver estructura de ns (pulso 2 ns / periodo 8 ns)
// — eso exige un osciloscopio. Solo mejora la envolvente (~104 ns) y amplitud.
//
// Lectura: pipenv run python mac_test/window_live.py  (FS_MHZ=54)
// =============================================================================
module top (
    input  wire sys_clk,            // 27 MHz cristal (pin 52) → PLL
    ...
    // ── PLL 27 → 54 MHz (verificado en HW, era el FW 0x0005) ──────────────────
    wire clk;
    wire pll_lock;
    rPLL #(
    ...
    always @(posedge clk) adc_reg <= adc_data_raw[11:2];
    ...
    reg [22:0] evt = 0;   // estira flanco a ~0.1 s @ 54 MHz (5_400_000)
    always @(posedge clk) begin
